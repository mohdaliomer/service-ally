    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">{filtered.length} complaints found</p>
        </div>
      ...
      <Card>
        <CardContent className="pt-4 pb-4 flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <Input
              placeholder="Search by ID, description, store..."
              value={search}
                ...
                {filtered.map((c) => (
                  <tr key={c.id} className="border-b last:border-0 hover:bg-muted/30 transition-colors">
                    ...
                    <td className="py-3 px-4 text-xs text-muted-foreground whitespace-nowrap">
                      {new Date(c.dateTime).toLocaleDateString('en-IN', { day: '2-digit', month: 'short' })}
                    </td>
                    ...
                    <td className="py-3 px-4 text-xs text-muted-foreground">{c.assignedTo || '—'}</td>
                  </tr>
                ))}
                {filtered.length === 0 && (
                  <tr>
